...
apply Z.le_trans with (Z.abs a * Z.abs a + 0 * Z.abs b); auto with zarith.
Qed.
...
intros H3 H4.
apply Z.le_lt_trans with (Z.abs a * Z.abs a + 0 * Z.abs b); auto with zarith.
...
apply Z.le_trans with x2; auto with zarith.
rewrite Zabs_non_eq; auto with zarith.
apply Z.le_trans with x1; auto with zarith.
...
apply Z.le_lt_trans with (Z.sqrt p * Z.sqrt p); auto with zarith.
assert (Hc1: Z.abs (y2 - y1) <= Z.sqrt p); auto with zarith.
...
rewrite <- Ha1; apply Zplus_lt_compat;
 apply Z.le_lt_trans with (Z.sqrt p * Z.sqrt p); auto with zarith.
apply Z.le_trans with (Z.sqrt p * d); auto with zarith.
apply Z.le_trans with (Z.sqrt p * c); auto with zarith.
case (Zle_lt_or_eq 1 q); auto with zarith.
...
auto with zarith.
pattern (2 * q) at 2; replace (2 * q) with (Z.succ (Z.pred (2 * q)));
 auto with zarith.
...
assert (Hp: 1 < p).
inversion H1; auto.
case (Z.eq_dec 0 (Zmod a p)); intros H4.
exists (Z.div a p).
...
rewrite (Zmult_comm (Z.div n p)); rewrite <- H; auto with zarith.
...
apply Z.ge_le; apply (Z_div_ge0 (Z.abs a) p); intros; (try apply Z.le_ge);
 auto with zarith.
assert (Hb1: 0 <= Z.abs b / p).
apply Z.ge_le; apply (Z_div_ge0 (Z.abs b) p); intros; (try apply Z.le_ge);
 auto with zarith.
...
apply Z.lt_le_trans with ((Z.abs a / p) * (Z.abs a / p) + 0 * (Z.abs b / p));
 auto with zarith.
rewrite Zmult_0_l; rewrite Zplus_0_r; replace 0 with (0 * (Z.abs a / p));
 auto with zarith.
apply Zmult_lt_compat_r; auto with zarith.
